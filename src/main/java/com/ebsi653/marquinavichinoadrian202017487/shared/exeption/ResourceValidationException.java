package com.ebsi653.marquinavichinoadrian202017487.shared.exeption;
...
public class ResourceValidationException extends RuntimeException {
	...
	public <T> ResourceValidationException(String resourceName, Set<ConstraintViolation<T>> violations) {
		super(String.format("Not all constraints satisfied for %s: %s", resourceName, violations.stream().map(
				violation -> String.format("%s %s", violation.getPropertyPath(), violation.getMessage())).collect(
				Collectors.joining(", "))));
	}
